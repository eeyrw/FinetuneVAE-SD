...
class FinetuneFaceData(Dataset):
    ...
    def __init__(
        ...
        self.img_list = [
            os.path.join(self.data_dir, imageInfo['IMG'])
            for imageInfo in self.imageInfoList
        ]

        self.size = size
        self.transform = transforms.Compose([
            transforms.Resize(prescale_size),
...
class DataModule():
    ...
    def __init__(self, data_dir, batch_size=64, val_size=0.1, prescale_size=768,size=384):
        super().__init__()
        self.data_dir = data_dir
        self.batch_size = batch_size
        self.val_size = val_size
        self.size = size
        self.prescale_size = prescale_size

        self.ds = FinetuneFaceData(self.data_dir, self.prescale_size,self.size)
        train_size = int(len(self.ds) * (1 - self.val_size))
        validate_size = len(self.ds)-train_size
        self.train_dataset, self.validate_dataset = torch.utils.data.random_split(
            self.ds, [train_size, validate_size])

        self.train_loader = DataLoader(self.train_dataset,
                                       batch_size=1,
                                       shuffle=True,
                                       num_workers=3)
        self.validate_loader = DataLoader(self.validate_dataset,
                                          batch_size=1,
                                          shuffle=True,
                                          num_workers=3)

        print(
            f"Train size: {len(self.train_dataset)}, Val size: {len(self.validate_dataset)}"
        )
...
def do_validation(
    ...
    for step, batch in enumerate(val_dataloader):
        target, name = batch
        pred = vae(target, sample_posterior=True).sample
        # kl_loss = posterior.kl()
        # kl_loss = kl_loss.mean() # torch.sum(kl_loss) / kl_loss.shape[0]
        rec_loss = torch.abs(target - pred)
        rec_loss = rec_loss.mean(
        )  # torch.sum(rec_loss) / (rec_loss.shape[0] *  rec_loss.shape[2] * rec_loss.shape[3])
        lpips_loss = lpips_loss_fn(pred, target).mean()
        loss = rec_loss + lpips_loss_weight * lpips_loss  # + self.kl_weight * kl_loss
    ...
    rec_loss = torch.stack(rec_loss_list).mean()
    lpips_loss = torch.stack(lpips_loss_list).mean()
    ...
    with autocast_ctx:
        for img1, img2, name in zip(target_list, pred_list, name_list):
            ...
            images.append(img)
            # os.makedirs(self.log_dir + "/" + str(self.current_epoch),
            #             exist_ok=True)
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "wandb":
            tracker.log(
                {
                    phase_name: [
                        wandb.Image(image, caption=f"{name}") for image, name in zip(images,name_list)
                    ]
                }
            )
    return images
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=768,
        help=(
            "The prescale resolution for input images, all the images in the train/validation dataset will be resized to this"
            " resolution"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument("--sgd_weight_decay", type=float, default=5e-4, help="Weight decay to use of SGD.")
    parser.add_argument("--sgd_momentum", type=float, default=0.9, help="momentum value for the SGD optimizer")
    ...
    parser.add_argument(
        ...
        type=str,
        default='ADAMW',
        help="Could be SGD,ADAMW,ADAMW8",
    )
    parser.add_argument(
        "--report_to",
        type=str,
    ...
    parser.add_argument('--full_finetune', type=bool_t, default='False',
                        help='Finetune whole vae rather than vae decoder only')
...
def main():
    ...
    vae = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path,
                                        cache_dir=args.cache_dir, subfolder="vae", 
    ...
    if args.optimizer == 'ADAMW8':
        ...
        raise NotImplementedError(f'No implementation of {args.optimizer}')

    if args.optimizer == 'ADAMW' or args.optimizer == 'ADAMW8':
        optimizer = optimizer_cls(
            vae.decoder.parameters() if not args.full_finetune else vae.parameters(),
        ...
        optimizer = optimizer_cls(vae.decoder.parameters() if not args.full_finetune else vae.parameters(),
            lr=args.learning_rate, 
    ...
    data_module = DataModule(args.train_data_dir,
                             batch_size=args.train_batch_size,
                             val_size=0.00001,
                             prescale_size=args.prescale_resolution,
                             size=args.resolution)
    train_dataloader = data_module.train_loader
    val_dataloader = data_module.validate_loader
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("vae-fine-tune", config=vars(args))

    # Train!
    total_batch_size = args.train_batch_size * accelerator.num_processes * args.gradient_accumulation_steps

    logger.info("***** Running training *****")
    logger.info(f"  Num examples of training = {len(data_module.train_dataset)}")
    logger.info(f"  Num examples of validation = {len(data_module.validate_dataset)}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(vae):
                ...
                rec_loss = torch.abs(target - pred)
                if epoch < args.num_train_epochs // 3 * 2:
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        unwrapped_vae = unwrap_model(vae)
        ...
        if epoch == args.num_train_epochs // 3 * 2:
            lpips_loss_weight = lpips_loss_weight * 0.1

        if accelerator.is_main_process:
            if epoch % args.validation_epochs == 0 or args.debug:
                pass
                # unwrapped_vae = unwrap_model(vae)
                # unwrapped_vae.eval()
                # images = do_validation(unwrapped_vae, 
                #                        val_dataloader,
                #                        lpips_loss_fn,
                #                        lpips_loss_weight,
                #                        args, accelerator, epoch)
                # unwrapped_vae.train()
